...
const ConsultantChat = () => {
    ...
    const [mail, setMail] = useLocalStorage('User_Detail', []);
            ...
            <Container maxWidth="md" style={{ marginTop: '20px' }}>
                <div style={{ backgroundColor: '#fff', padding: '24px', borderRadius: '8px', boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.1)' }}>
                    <Card className="container mt-5">
                        <CardContent>
                            <Typography variant="h5" style={{ fontWeight: 'bold' }}>Chat With User</Typography>
                                ...
                                <List>
                                    {messages.map(message => (
                                        <ListItem key={message.id}>
                                            <ListItemText primary={<Typography variant="body1" style={{ fontWeight: 'bold' }}>{message.mail} : </Typography>} secondary={<Typography variant="body1">{message.message}</Typography>} />
                                        </ListItem>
                                    ))}
                                </List>
                            </div>
